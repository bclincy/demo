...
Vagrant.configure("2") do |config|
	...
	config.vm.provision "shell", inline: <<-SHELL
		...
		apt-get install -y build-essential software-properties-common nodejs php7.2 php7.2-mysqlnd php7.2-curl \
			php7.2-zip php7.2-mbstring php7.2-xml php7.2-xdebug
	...
	config.vm.provision "shell", privileged: false, inline: <<-SHELL
		...
		echo "  -> NodeJS 10.x"
		echo "  -> PHP 7.2 (with extensions: mysqlnd, curl, zip, mbstring, xml, xdebug)"
		echo "  -> Composer"
		echo "  -> MariaDB"
		echo
		echo "Mapped Ports:"
		echo "  -> VM:80 > Host:8000"
